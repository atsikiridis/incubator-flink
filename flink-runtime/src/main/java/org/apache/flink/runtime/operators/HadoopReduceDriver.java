...
public class HadoopReduceDriver<IT, OT> implements PactDriver<GroupReduceFunction<IT, OT>, OT> {
	...
	public void prepare() throws Exception {
		...
		if (config.getDriverStrategy() != DriverStrategy.HADOOP_REDUCE) {
			throw new Exception("Unrecognized driver strategy for HadoopReduce driver: " + config.getDriverStrategy().name());
		}
